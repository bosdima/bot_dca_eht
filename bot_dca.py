...
class BybitClient:
    ...
    # =============================================================================
    # ИСПРАВЛЕННАЯ ФУНКЦИЯ v5.28.1: округляет ВВЕРХ для продажи ВСЕГО баланса
    # =============================================================================
    def _round_quantity_for_sell(self, quantity: float, qty_step: float = None,
        ...
        ИСПРАВЛЕНИЕ v5.28.1: Теперь округляет ВВЕРХ до ближайшего кратного qty_step,
        если превышение составляет менее 1% от баланса.
        Это позволяет продать ВЕСЬ баланс монет.
        
        Пример: баланс 0.01996984 ETH с qty_step=0.01
        - Округление вниз: 0.01 (остаток 0.00996984)
        - Округление вверх: 0.02 (превышение 0.00003016 = 0.15% от баланса)
        - Результат: 0.02 ✅
        """
        if quantity <= 0:
            return 0.0
        ...
        if qty_step and qty_step > 0:
            ...
            # ⭐ ГЛАВНОЕ ИСПРАВЛЕНИЕ v5.28.1:
            # Если есть остаток, пробуем округлить ВВЕРХ
            if remainder > 0:
                rounded_up = floor_qty + qty_step
                # Насколько округлённое вверх значение превышает баланс
                overage = rounded_up - quantity
                
                # Если превышение меньше 1% от баланса, округляем ВВЕРХ
                # Биржа Bybit допускает такую погрешность
                if overage < quantity * 0.01:
                    return round(rounded_up, decimal_places)
            
            # Иначе используем округление вниз (безопасный режим)
