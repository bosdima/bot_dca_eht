...
class BybitClient:
    ...
    # =============================================================================
    # ИСПРАВЛЕННАЯ ФУНКЦИЯ v5.28.0: теперь округляет ВВЕРХ для продажи ВСЕГО баланса
    # =============================================================================
    def _round_quantity_for_sell(self, quantity: float, qty_step: float = None,
        ...
        ИСПРАВЛЕНИЕ v5.28.0: Теперь округляет ВВЕРХ до ближайшего кратного qty_step,
        если остаток после округления вниз составляет менее 1% от qty_step.
        Это позволяет продать ВЕСЬ баланс монет (например, 0.01996984 ETH → 0.02 ETH).
        """
        if quantity <= 0:
            return 0.0
        ...
        if qty_step and qty_step > 0:
            ...
            # ⭐ ГЛАВНОЕ ИСПРАВЛЕНИЕ: если остаток меньше 1% от qty_step,
            # округляем ВВЕРХ — это позволяет продать ВЕСЬ баланс
            if remainder > 0 and remainder < (qty_step * 0.01):
                rounded = floor_qty + qty_step
                # Проверяем, что округлённое значение не превышает баланс более чем на 1%
                if rounded <= quantity * 1.01:
                    return round(rounded, decimal_places)
