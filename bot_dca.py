...
Версия 5.6.5 (24.06.2026)
ИСПРАВЛЕНИЯ:
- Исправлено округление количества для продажи - теперь используется round вместо floor
- Увеличена точность отображения количества до 8 знаков
- Исправлен force_check_completed_sells для корректного поиска завершенных продаж
- Исправлен расчет объема продажи - теперь используется ФАКТИЧЕСКИЙ БАЛАНС с корректным округлением
...
MAIN_MENU_BUTTONS = [
    ...
    "?? Редактировать покупки", "?? Настройки", "?? Статус бота",
...
class BybitClient:
    ...
    def _round_quantity_by_step(self, quantity: float, qty_step: float, min_qty: float) -> float:
        """
        Округление количества с учетом шага.
        Используется round вместо floor для корректного сохранения полного объема.
        """
        if qty_step <= 0:
        ...
        # Используем round вместо floor для сохранения полного объема
...
class DCAStrategy:
    ...
    async def _send_sell_order_notification(self, symbol: str, quantity: float, price: float, profit_percent: float, avg_price: float, bot):
        ...
        message = (
            ...
            f"?? Пара: `{symbol}`\n"
            f"?? Количество: `{format_quantity(quantity, 8)}` {coin}\n"
    ...
    async def check_and_create_sell_order(self, symbol: str, bot, silent: bool = False) -> Dict:
        try:
            ...
            # Используем ФАКТИЧЕСКИЙ БАЛАНС для продажи с корректным округлением
    ...
    async def sell_order_check_loop(self, symbol: str, user_id: int, bot):
        ...
        while True:
            try:
                ...
                # Проверяем, есть ли статистика DCA
                stats = self.db.get_dca_stats(symbol)
                if not stats or stats['total_quantity'] <= 0:
                    logger.info(f"No DCA stats for {symbol}, skipping")
                    continue
    ...
    async def execute_scheduled_purchase(self, symbol: str, profit_percent: float, bot) -> Dict:
        ...
        if stats and stats['total_quantity'] > 0:
            ...
            if current_price > avg_price:
                ...
                return {
                    ...
                    'message': f'?? Покупка пропущена: {reason}'
                }
        ...
        if result['success']:
            ...
            if total_quantity_for_sell <= 0:
                ...
                result['sell_warning'] = f"?? Монеты не зачислены на баланс. Ордер на продажу не создан."
            ...
            # Создаем ордер на продажу, используя ФАКТИЧЕСКИЙ БАЛАНС с корректным округлением
            ...
            if sell_result['success']:
                ...
                result['sell_warning'] = f"?? Ордер на продажу отложен"
            ...
            if current_price >= order['target_price']:
                ...
                if sell_result['success']:
                    ...
                    msg = (f"? *ОТЛОЖЕННЫЙ ОРДЕР ВЫПОЛНЕН!*\n\n"
                           f"?? Токен: `{symbol}`\n"
                           f"?? Количество: `{format_quantity(order['quantity'], 8)}`\n"
    ...
    def _format_sell_notification(self, sell: Dict, symbol: str) -> str:
        ...
        message += f"?? Количество: <code>{format_quantity(sell['quantity'], 8)}</code>\n"
        ...
        message += f"?? <b>ВНИМАНИЕ: ID покупок будут сброшены и начнутся с 1!</b>"
    ...
    async def place_full_sell_order(self, update, symbol: str, profit_percent: float, auto_cancel_old: bool = True) -> Dict:
        try:
            ...
            if order_value < min_amt:
                ...
                msg = (f"? *ОРДЕР ОТЛОЖЕН*\n\n"
                       f"?? Токен: `{symbol}`\n"
                       f"?? Количество: `{format_quantity(sell_qty, 8)}` {coin}\n"
            ...
            if update and hasattr(update, 'message'):
                await update.message.reply_text(f"?? Выставляю ордер на продажу {format_quantity(sell_qty, 8)} {coin} по {format_price(rounded_price, 4)} USDT...")
            ...
            if result['success']:
                ...
                self.db.log_action('FULL_SELL_ORDER', symbol, f"Ордер на продажу {sell_qty:.8f} {coin} по {rounded_price:.4f} USDT")
                
                warning_msg = ""
                if sell_qty < stats['total_quantity']:
                    diff = stats['total_quantity'] - sell_qty
                    warning_msg = f"\n?? Продано только {format_quantity(sell_qty, 8)} из {format_quantity(stats['total_quantity'], 8)} {coin}."
                ...
                msg = (f"? *ОРДЕР ОТЛОЖЕН*\n\n"
                       ...
                       f"?? Целевая цена: `{format_price(rounded_price, 4)}` USDT\n"
                       f"?? Целевая прибыль: `{profit_percent}%`\n\n"
                       f"?? *Недостаточно средств на балансе (баланс обновляется)*\n\n"
...
class FastDCABot:
    ...
    def get_main_keyboard(self):
        ...
        keyboard = [
            ...
            [KeyboardButton("? Добавить покупку вручную"), KeyboardButton("?? Редактировать покупки")],
            [KeyboardButton("?? Настройки"), KeyboardButton("?? Управление ордерами")],
    ...
    def get_settings_keyboard(self):
        ...
        keyboard = [
            ...
            [KeyboardButton("?? Сумма для ручного ордера"), KeyboardButton("?? Настройки отслеживания")],
    ...
    def get_symbol_selection_keyboard(self):
        ...
        keyboard.append([KeyboardButton("?? Ввести свой токен")])
    ...
    def get_purchases_list_keyboard(self, purchases):
        ...
        for p in purchases:
            ...
            btn_text = f"ID{p['id']}: {date_display} - {format_quantity(p['quantity'], 8)} по {format_price(p['price'], 4)}"
    ...
    async def back_to_settings_from_purchase(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        await update.message.reply_text("?? *Настройки*", reply_markup=self.get_settings_keyboard(), parse_mode='Markdown')
        return ConversationHandler.END
    ...
    async def handle_sell_confirmation(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if text == "? Да, выставить ордер на продажу":
            ...
            if result['success']:
                ...
                self.db.log_action('SELL_ORDER_PLACED', sell_data['symbol'], f"Ордер на продажу {result['quantity']:.8f} по {result['price']:.4f} USDT")
    ...
    async def test_tracking(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if buy_result['missing'] or sell_result['missing']:
            summary += f"\n\n?? *Найдены новые ордера!* Сейчас пришлю уведомления..."
        ...
        for sell in sell_result['missing']:
            ...
            msg_text = (f"?? *СДЕЛКА ПРОДАНА!*\n\n"
                       f"?? Токен: `{symbol}`\n"
                       f"?? Количество: `{format_quantity(sell['quantity'], 8)}`\n"
    ...
    async def show_portfolio(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if coin_balance and 'equity' in coin_balance:
                ...
                message += f"?? *{coin}*\n"
                message += f"Количество: `{format_quantity(equity, 8)}`\n"
    ...
    async def toggle_dca(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if is_active:
            ...
            if result.get('success'):
                ...
                await update.message.reply_text(
                    f"?? *DCA ЗАПУЩЕН, НО ОРДЕР НЕ СОЗДАН*\n\n"
                    f"?? Токен: `{symbol}`\n"
    ...
    async def process_symbol_selection(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if text == "? Отмена":
            await update.message.reply_text("? Отменено", reply_markup=self.get_settings_keyboard())
            return SELECTING_ACTION
        if text == "?? Ввести свой токен":
            await update.message.reply_text("?? Введите символ токена (например: TONUSDT):", reply_markup=self.get_cancel_keyboard())
    ...
    async def _validate_and_set_symbol(self, update: Update, symbol: str) -> int:
        ...
        await update.message.reply_text(
            ...
            f"?? Минимальная сумма для Авто DCA: {min_amt} USDT",
            reply_markup=self.get_settings_keyboard()
        )
        return SELECTING_ACTION
    ...
    async def reset_ladder(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text("?? Статистика DCA очищена! Лестница сброшена.\n?? ID покупок будут начинаться с 1 при следующем добавлении.", reply_markup=self.get_ladder_settings_keyboard())
        return LADDER_MENU
            ...
            if result['success']:
                ...
                if purchase_id:
                    ...
                    if sell_result['success']:
                        ...
                        await update.message.reply_text(f"?? Не удалось создать ордер на продажу: {sell_result.get('error', 'Unknown')}")
                    msg = f"? *Лимитный ордер создан!*\n\nЦена: `{format_price(price, 4)}` USDT\nСумма: `{amount:.2f}` USDT\nКоличество: `{format_quantity(result['quantity'], 8)}`\n"
                    if drop_percent > 0:
    ...
    async def edit_purchases_list(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not purchases:
            ...
            return ConversationHandler.END
        await update.message.reply_text("?? Выберите покупку:", reply_markup=self.get_purchases_list_keyboard(purchases))
    ...
    async def edit_purchase_selected(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            await update.message.reply_text(f"?? *РЕДАКТИРОВАНИЕ ID: {purchase_id}*\n\n?? Дата: `{date_display}`\n?? Цена: `{format_price(purchase['price'], 4)}` USDT\n?? Количество: `{format_quantity(purchase['quantity'], 8)}`", reply_markup=self.get_edit_purchases_keyboard(), parse_mode='Markdown')
            return EDIT_PURCHASE_SELECT
        except Exception as e:
    ...
    async def delete_purchase_confirm(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        await update.message.reply_text("?? *Удалить эту покупку?*", reply_markup=self.get_confirm_delete_keyboard(), parse_mode='Markdown')
                    ...
                    if result['success']:
                        if self.authorized_user_id:
                            ...
                            if result.get('sell_quantity'):
                                msg += f"?? Ордер на продажу: `{format_quantity(result['sell_quantity'], 8)}` {symbol.replace('USDT', '')}\n"
    ...
    async def skip_clear_stats(self, update: Update, context: ContextTypes.DEFAULT_TYPE, symbol: str, sell_id: int):
        query = update.callback_query
        self.db.mark_completed_sell_notified(sell_id)
        await query.edit_message_text(f"? Очистка статистики для {symbol} отложена.\n\n?? Статистика DCA сохранена.\n?? Вы можете очистить её позже вручную через раздел '?? Редактировать покупки' или '?? Лестница Мартингейла' > 'Сбросить лестницу'.", parse_mode='Markdown')
    ...
    async def confirm_clear_stats(self, update: Update, context: ContextTypes.DEFAULT_TYPE, symbol: str):
        ...
        msg = (f"?? *ПОДТВЕРЖДЕНИЕ ОЧИСТКИ*\n\n"
               ...
               f"?? ID покупок будут сброшены и начнутся с 1!\n\n"
    ...
    async def add_executed_order_to_stats(self, update: Update, context: ContextTypes.DEFAULT_TYPE, order_id: str):
        ...
        if order_dict.get('added_to_stats', 0) == 1:
            await update.callback_query.edit_message_text("?? Этот ордер уже был добавлен в статистику.")
            return
        
        if self.db.is_order_already_added(order_id):
            await update.callback_query.edit_message_text("?? Этот ордер уже есть в статистике покупок (по ID ордера).")
        ...
        if purchase_id:
            ...
            await update.callback_query.edit_message_text("?? Ордер уже был добавлен в статистику ранее.")
        else:
    ...
    async def send_sell_recommendation_from_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        msg = (f"?? *РЕКОМЕНДАЦИЯ ПО ПРОДАЖЕ*\n\n"
               f"?? Токен: `{symbol}`\n"
               f"?? Количество для продажи: `{format_quantity(display_quantity, 8)}` {coin}\n"
               f"?? Целевая прибыль: `{profit_percent}%`\n"
    ...
    def setup_handlers(self):
        ...
        tracking_conv = ConversationHandler(
            entry_points=[MessageHandler(filters.Regex('^(?? Настройки отслеживания)$'), self.tracking_settings)],
        ...
        edit_purchases_conv = ConversationHandler(
            entry_points=[MessageHandler(filters.Regex('^(?? Редактировать покупки)$'), self.edit_purchases_list)],
        ...
        main_conv = ConversationHandler(
            entry_points=[MessageHandler(filters.Regex('^(?? Настройки)$'), self.settings_menu)],
            states={
                SELECTING_ACTION: [
                    ...
                    MessageHandler(filters.Regex('^(?? Настройки отслеживания)$'), self.tracking_settings),
        ...
        self.application.add_handler(MessageHandler(filters.Regex('^(?? Настройки)$'), self.settings_menu))
